...
function reduce(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actions.STORE_CLEARANCE_DECLARE_REQUEST_RESET:
            ...
            return nextState || state;
        // ======================================================== Cancel clearance
        // Resolve event to set cancel clearance init request store data
        case actions.STORE_CANCEL_CLEARANCE_REQUEST_INIT:
            nextState = {...state, cancel: requestInitValue()};
            return nextState || state;
        // Resolve event to set cancel clearance failed request store data
        case actions.STORE_CANCEL_CLEARANCE_REQUEST_FAILED:
            nextState = {...state, cancel: requestFailedValue(action.message)};
            return nextState || state;
        // Resolve event to set cancel clearance succeeded request store data
        case actions.STORE_CANCEL_CLEARANCE_REQUEST_SUCCEEDED:
            nextState = {...state, cancel: requestSucceededValue(action.message)};
            return nextState || state;
        // Resolve event to set cancel clearance reset request store data
        case actions.STORE_CANCEL_CLEARANCE_REQUEST_RESET:
