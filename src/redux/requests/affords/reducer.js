...
function reduce(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actions.STORE_ADD_AFFORD_REQUEST_RESET:
            ...
            return nextState || state;
        // ======================================================== Cancel afford
        // Resolve event to set cancel afford init request store data
        case actions.STORE_CANCEL_AFFORD_REQUEST_INIT:
            nextState = {...state, cancel: requestInitValue()};
            return nextState || state;
        // Resolve event to set cancel afford failed request store data
        case actions.STORE_CANCEL_AFFORD_REQUEST_FAILED:
            nextState = {...state, cancel: requestFailedValue(action.message)};
            return nextState || state;
        // Resolve event to set cancel afford succeeded request store data
        case actions.STORE_CANCEL_AFFORD_REQUEST_SUCCEEDED:
            nextState = {...state, cancel: requestSucceededValue(action.message)};
            return nextState || state;
        // Resolve event to set cancel afford reset request store data
        case actions.STORE_CANCEL_AFFORD_REQUEST_RESET:
